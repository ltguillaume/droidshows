...
public class JsonStore
{
	...
	public JSONObject createJSONSerie(List<Serie> series) {
		...
		try {
			...
			for (int i = 0; i < series.size(); i++) {
				...
				// adicionar os episodios JSONArray de JSONObject de Episodios .toString() (?)
	...
	public void writeJSONFile(Context context, List<Serie> series) {
		...
		try {
			...
			osw.flush();
			Log.i(TAG, "JSON Data saved.");
		} catch (Exception e) {
			// e.printStackTrace();
			Log.e(TAG, "JSON Data not saved.");
		} finally {
			try {
				osw.close();
				fOut.close();
	...
	public List<Serie> readJSONFile(Context context) {
		...
		try {
			...
			if (!jsonfile.exists()) {
				...
				// Log.i("DroidSeries", "json file lenght: " +
				// jsonfile.length());
